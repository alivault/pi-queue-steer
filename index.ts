...
const SELECT_PREVIOUS_ROW_KEY = "up";
...
class QueueTimelineWidget implements Component {
	...
	private renderLaneBox(
		...
		const state = this.paused ? "paused" : laneHeld ? "held while editing" : undefined;
		const name = lane === "steer" ? "steer" : "follow-up";
		const shortTitle = ` ${name} (${items.length}) `;
		const fullTitle = state ? ` ${name} (${items.length}) · ${state} ` : shortTitle;
...
export default function queueSteerExtension(pi: ExtensionAPI) {
	 ...
	 *
	 * Draft reordering is shown immediately while the committed queues remain
	 * unchanged until save. Held flags follow dispatch truth: they reflect each
	 * row's committed lane, so an uncommitted move never changes delivery.
	 */
		...
		const factory = ((tui, theme, keybindings) => {
			...
			editor.handleInput = (data: string): void => {
				if (editSession) {
					...
					if (matchesKey(data, MOVE_NEXT_ROW_KEY)) {
				...
				if (queue.length > 0 && matchesKey(data, MOVE_PREVIOUS_ROW_KEY)) {
					selectQueueItem(ctx, "previous");
					return;
				}
				if (
